...
class RefreshRightContainer: UIView {
    ...
    override func willMove(toSuperview newSuperview: UIView?) {
        ...
        addObservers()
        frame = CGRect(x: attachedScrollView.contentSize.width, y: 0, width: frame.width, height: frame.height)
    }
    ...
    func handleScrollOffSetChange(_: [NSKeyValueChangeKey: Any]?) {
        if state == .refreshing {
            return
        }
        ...
        if attachedScrollView.isDragging {
            let percent = (offSetX + scrollViewWidth - contentInset.left - contentWidth) / frame.width
    ...
    func handleContentSizeChange(_: [NSKeyValueChangeKey: Any]?) {
        frame = CGRect(x: attachedScrollView.contentSize.width, y: 0, width: frame.size.width, height: frame.size.height)
    }
