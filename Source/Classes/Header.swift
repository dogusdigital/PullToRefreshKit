...
//        spinner.center = imageView.center
    ...
    public func percentUpdateDuringScrolling(_: CGFloat) {
        isHidden = false
    }
    ...
    public func stateDidChanged(_ oldState: RefreshHeaderState, newState: RefreshHeaderState) {
        if oldState == RefreshHeaderState.idle && newState == RefreshHeaderState.pulling {
            ...
            guard imageView.transform == CGAffineTransform.identity else {
                return
            }
        ...
        if oldState == RefreshHeaderState.pulling && newState == RefreshHeaderState.idle {
            textLabel.text = textDic[.pullToRefresh]
            guard imageView.transform == CGAffineTransform(rotationAngle: -CGFloat.pi + 0.000001) else {
                return
            }
            UIView.animate(withDuration: 0.4, animations: {
...
open class RefreshHeaderContainer: UIView {
    ...
    func handleScrollOffSetChange(_ change: [NSKeyValueChangeKey: Any]?) {
        var insetHeight: CGFloat! = delegate?.heightForRefreshingState?()
        if insetHeight == nil {
            insetHeight = delegate?.heightForHeader()
        }
        var fireHeight: CGFloat! = delegate?.heightForFireRefreshing?()
        if fireHeight == nil {
            fireHeight = delegate?.heightForHeader()
        }
        if state == .refreshing {
        ...
        if let oldOffset = (change?[NSKeyValueChangeKey.oldKey] as AnyObject).cgPointValue {
            ...
            if oldPercent >= 1.0 && percent == 0.0 {
                ...
                delegate?.percentUpdateDuringScrolling?(percent)
            }
        } else {
            delegate?.percentUpdateDuringScrolling?(percent)
        }
    ...
    func endRefreshing(_ result: RefreshResult, delay: TimeInterval = 0.0) {
        delegate?.didBeginHideAnimation(result)
        delayTimer = Timer(timeInterval: delay, target: self, selector: #selector(RefreshHeaderContainer.updateStateToIdea), userInfo: nil, repeats: false)
        RunLoop.main.add(delayTimer!, forMode: RunLoopMode.commonModes)
    }
